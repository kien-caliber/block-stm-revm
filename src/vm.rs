...
use crate::{
    mv_memory::MvMemory, storage::StorageRefToDatabaseWrapper, EvmAccount, MemoryEntry,
    MemoryLocation, MemoryLocationHash, MemoryValue, ReadError, ReadLocations, ReadOrigin, ReadSet,
...
impl<'a, S: Storage> Vm<'a, S> {
    pub(crate) fn new(
        ...
        if chain.is_optimism() {
            let mut db = StorageRefToDatabaseWrapper(storage);
