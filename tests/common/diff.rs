...
impl<T: Diffable, E: Debug + PartialEq> Diffable for Result<T, E> {
    fn diff(path: PathBuf, left: &Self, right: &Self) -> Vec<Reason> {
        match (left, right) {
            ...
            (Ok(_), Err(err)) => {
                Vec::from(&[Reason::new(path, "Ok(_)", format!("Err({:?})", err))])
            }
            (Err(err), Ok(_)) => {
                Vec::from(&[Reason::new(path, format!("Err({:?})", err), "Ok(_)")])
            }
...
impl<K: Copy + Ord + Debug + Hash, V: Diffable + Clone, S: BuildHasher> Diffable
...
impl_diffable_complex!(EvmAccount; balance, nonce, code_hash, code, storage);
